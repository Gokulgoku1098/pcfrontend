  ...
  const loaddata = async () => {
    try {
      ...
      const filtered = result.data.filter(
        (item) => item.category === "cpucoller")
      setproduct(filtered)
      console.log(product);
    }
    catch (err) {
      console.log("coloor error", err);
  ...
  return (
    <>
      ...
      h-20 w-full bg-gradient-to-r from-black via-gray-900 via-yellow-500 via-pink-600 to-purple-900">
        <h1 className="text-3xl text-white">Select your CPU Cooler</h1>
      </div>

      {product.map((cooler, i) => (
        <div
          onClick={() => { handleNav(cooler._id)}}
          key={i}
        ...
        >
          ...
          <div className="flex-1">
            <h1 className="font-semibold text-lg md:text-xl">{cooler.name}</h1>
